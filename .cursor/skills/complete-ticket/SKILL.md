...
Process mirrored from [davgor.github.io](https://github.com/davgor/davgor.github.io); Electron/deploy expectations from [AI-DND-Matrix](https://github.com/davgor/AI-DND-Matrix).
...
This repo's standing rule: tests are written before the implementation that satisfies them, for main/preload/renderer logic, shared helpers, and any other code with testable behavior. For each acceptance criterion that names a behavior:
...
UI-only criteria (visual layout, manual interaction flows) don't need a test-first cycle unless the criterion itself says "tested" — but still implement them carefully against the criterion's exact wording. Prefer Vitest + Testing Library for unit/component coverage.

Respect the standing engineering rules while you work:
- TypeScript strict mode, no `any` escapes used to dodge a type problem.
- Lint rules apply (`npm run lint`, zero warnings). **Never relax, override, or disable a lint rule to make your code pass — fix the code.** If a rule seems genuinely wrong for a specific case, stop and ask the user before touching the lint config.
- After code edits, follow `.ai-instructions.md` (lint:fix / format when those scripts exist).
- Electron security baseline (contextIsolation/sandbox on, narrow typed IPC) is never weakened by a ticket's implementation.
...
**CI parity:** Confirm the equivalent local commands for `.github/workflows/pr-checks.yml` and `deadcode.yml` all pass before calling substantial work done.

**Native modules / Electron** (`main`/`preload` or compiled `.node` deps): passing `npm test` under system Node is not enough. Rebuild for Electron's ABI and exercise the new path in the real app before considering the ticket done. Skip only for tickets that add no native module and don't touch `main`/`preload` — say so rather than running it pointlessly.
...
4. **Fan out independent sub-tickets to subagents.** For each parallelizable group, launch one `Task` subagent per sub-ticket (or a small cluster of trivially related ones) in a *single message* so they run concurrently. Each subagent prompt must be self-contained:
   ...
   - The standing rules from section 3 (TDD-first, lint strict, no `any` escapes).
   - An instruction to self-check by running only its own new test file(s) (e.g. `npx vitest run src/foo.test.ts`), and explicitly **not** to run the full suite, lint, build, or CI — those are integration steps you run once, after every subagent reports back.
